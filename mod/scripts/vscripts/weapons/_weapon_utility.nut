...
void function VanguardEnergySiphon_DamagedPlayerOrNPC( entity ent, var damageInfo )
{
	...
	bool sameTeam = attacker.GetTeam() == ent.GetTeam()
	if ( sameTeam && !friendlyFireOn )
		return	
	
	// ff
	if ( friendlyFireOn )
	{
		if ( !sameTeam && forceHeal && hasEnergyTransfer )
			return
		if ( sameTeam && hasEnergyTransfer )
			return
	}
...
void function Elecriticy_DamagedPlayerOrNPC( entity ent, var damageInfo, asset humanFx, asset titanFx, float slowTurn, float slowMove )
{
	...
	if ( !IsValid( attacker ) ) // we added friendly fire, do a new check now!
		return
	if( attacker.GetTeam() == ent.GetTeam() && attacker != ent && !friendlyFireOn )
		return
